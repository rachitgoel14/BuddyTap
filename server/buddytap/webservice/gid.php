...
if (!empty($_POST)) {
    ...
    if (empty($_POST['username']) ||empty($_POST['gid'])||empty($_POST['groupname'])) {
        
        
        // Create some data that will be the JSON response 
        $response["success"] = 0;
        $response["message"] = "Group Id or Group Name cant be empty";
   ...
   /*  //if the page hasn't died, we will check with our database to see if there is
    ...
    $query        = " SELECT 1 FROM groupid WHERE gid = :gid";
    ...
    if ($row) {
        ...
        $response["success"] = 0;
        $response["message"] = "I'm sorry, this groupId is already in use";
        die(json_encode($response));
    } */
    ...
    $response["success"] = 1;
    $response["message"] = "Created Group Successfully !";
...
?>
	<h1>GroupId</h1> 
	<form action="gid.php" method="post"> 
	 Username:<br /> 
	  ...
	  GroupId:<br /> 
	    ...
	    <input type="submit" value="Enter Gid" /> 
	</form>
	<?php
}
